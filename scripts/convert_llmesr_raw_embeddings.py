"""Safely convert raw LLM-ESR item or user ndarray pickle assets to NPY."""
...
def main() -> None:
    ...
    parser.add_argument("--kind", choices=("item", "user"), default="item")
    ...
    asset_name = "itm_emb_np.pkl" if args.kind == "item" else "usr_emb_np.pkl"
    output_name = (
        "raw_item_embeddings.npy"
        if args.kind == "item"
        else "raw_user_embeddings.npy"
    )
    source = args.source or processed_dir / "author_assets" / asset_name
    output = args.output or processed_dir / output_name
    ...
    report_path = args.report or Path(
        f"reports/data/{args.dataset}_raw_{args.kind}_conversion.json"
    )
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    expected_hash = manifest["author_assets"][asset_name]["sha256"]
    array = decode_numpy_ndarray_pickle(
        source,
        expected_sha256=expected_hash,
    ).astype(np.float32)
    row_key = "items" if args.kind == "item" else "users"
    expected_rows = int(manifest["statistics"][row_key])
    if array.ndim != 2 or array.shape[0] != expected_rows:
        raise ValueError(
            f"raw {args.kind} matrix shape {array.shape} does not have "

...
def main() -> None:
    ...
    mapping = {(p["query_id"], p["variant"]): {x["identifier"]: x["candidate_id"] for x in p["candidate_mapping"]} for p in prompts}
    ...
    for line in args.results.open():
        ...
        grouped[row["query_id"]][row["variant"]] = {ids[k]: v for k, v in row["identifier_logits"].items()}
    ...
    relevance = {str(req): {str(r.passage_id): int(r.graded_relevance) for r in group.itertuples()} for req, group in qrels.groupby("request_id", sort=False)}
    ...
    for query_id, variants in grouped.items():
        ...
        bm25_rank = candidates[candidates["request_id"].astype(str) == request_id].sort_values("bm25_rank")["passage_id"].astype(str).tolist()
    ...
    for year in sorted(set(request_year.values())):
        ...
        report["by_year"][str(year)] = {metric: sum(row[metric][0] for row in subset) / len(subset) for metric in ("bm25", "first_baseline", "first_debiased")}

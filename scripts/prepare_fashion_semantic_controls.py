"""Create deterministic shuffled and moment-matched Fashion semantic controls."""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "data/processed/fashion_llmesr_author/pca64_item_embeddings.npy"
        ),
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("data/processed/fashion_llmesr_author/semantic_controls"),
    )
    ...
    report = {
        ...
        "controls": outputs,
        "definitions": {
            "shuffled": "row permutation preserving all source vectors exactly",
            "matched_random": (
                "Gaussian matrix standardized to each source dimension's mean and std"
            ),
        },
    }

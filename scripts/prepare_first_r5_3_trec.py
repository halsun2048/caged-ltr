...
def main() -> None:
    ...
    for query in queries:
        candidates = tuple(
            FirstCandidate(c.passage_id, c.passage, c.bm25_rank)
            for c in query.candidates[: args.candidate_limit]
        )
        for variant in VARIANTS:
            entries = build_prompt_entries(
                candidates, query_id=query.query_id, variant=variant, seed=42
            )
            prompt, token_count, word_budget = fit_first_prompt(
                tokenizer, query.query, entries, context_size=4096, initial_max_passage_words=300
            )
            mapping = [
                {
                    ...
                    "retrieval_rank": e.candidate.retrieval_rank,
                }
                for e in entries
            ]
            records.append(
    ...
    print(
        json.dumps({"queries": len(queries), "records": len(records), "output": str(args.output)})
    )

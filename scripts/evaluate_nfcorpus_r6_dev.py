...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--train',type=Path,required=True); ap.add_argument('--dev',type=Path,required=True); ap.add_argument('--gate-routes',type=Path,required=True); ap.add_argument('--mind-checkpoint',type=Path,required=True); ap.add_argument('--distilled-checkpoint',type=Path,required=True); ap.add_argument('--output',type=Path,required=True); ap.add_argument('--model',default='sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2'); ap.add_argument('--batch-size',type=int,default=32); ap.add_argument('--max-length',type=int,default=128); args=ap.parse_args()

...
def main():
    ...
    request_for_query = dict(zip(queries.query_id.astype(str), queries.request_id.astype(str), strict=True))
    year_for_request = dict(zip(queries.request_id.astype(str), queries.year.astype(int), strict=True))
    relevance = {str(req): {str(r.passage_id): int(r.graded_relevance) for r in g.itertuples()} for req, g in qrels.groupby("request_id", sort=False)}
    ...
    for year in sorted({x["year"] for x in values}):
        ...
        report["by_year"][str(year)] = {"queries": len(subset), "first_ndcg": sum(x["first"] for x in subset) / len(subset), "prp_ndcg": sum(x["prp"] for x in subset) / len(subset), "difference": sum(x["first"] - x["prp"] for x in subset) / len(subset), "paired_bootstrap_95ci": [sorted(year_diffs)[250], sorted(year_diffs)[9749]]}

"""Train a sharded English MiniLM bi-encoder on the frozen MIND R8.1 package."""
...
def read_shards(pattern: str | Path) -> pd.DataFrame:
    paths = sorted(Path(value) for value in glob.glob(str(pattern)))
...
def main() -> None:
    ...
    parser.add_argument(
        "--train", default="data/processed/mind_r8_1_v2/train_pairs/*.parquet"
    )
    parser.add_argument(
        "--dev", default="data/processed/mind_r8_1_v2/dev_listwise/*.parquet"
    )
    parser.add_argument("--model", default="sentence-transformers/all-MiniLM-L6-v2")
    parser.add_argument("--checkpoint", type=Path, default=Path("runs/mind_r8_2/latest.pt"))
    parser.add_argument(
        "--best-checkpoint", type=Path, default=Path("artifacts/mind_r8_2_large_student.pt")
    )
    parser.add_argument(
        "--report", type=Path, default=Path("reports/experiments/mind_r8_2_large_student.json")
    )
    ...
    parser.add_argument(
        "--negative-type", choices=("rank_hard", "random", "all"), default="all"
    )
    parser.add_argument(
        "--objective", choices=("pairwise", "pairwise_inbatch"), default="pairwise"
    )
    ...
    train = read_shards(args.train)
    dev = read_shards(args.dev)
    if args.negative_type != "all":
        train = train.loc[train.negative_type == args.negative_type].reset_index(drop=True)
        ...
        for batch_number in range(batch_offset, total_batches):
            ...
            with torch.autocast(
                ...
                if args.objective == "pairwise_inbatch":
                    ...
                    inbatch = nn.functional.cross_entropy(logits, labels)
                    loss = loss + args.inbatch_weight * inbatch
    ...
    payload = {
        "schema": "mind_r8_2_large_student_v1",
        "identity": run_identity,
        "language": "English",
        "model": args.model,
        "device": str(device),
        ...
        "boundaries": {
            ...
            "large_dev_used_for_early_stopping_only": True,
    ...
    args.report.with_suffix(".md").write_text(
        "# R8.2 large English MIND MiniLM student\n\n```json\n"

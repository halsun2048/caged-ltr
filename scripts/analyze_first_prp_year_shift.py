...
def main():
    ...
    text_len = dict(
        zip(
            queries.request_id.astype(str),
            queries["query"].astype(str).str.split().str.len(),
            strict=True,
        )
    )
    rel = {
        str(r): {str(x.passage_id): int(x.graded_relevance) for x in g.itertuples()}
        for r, g in qrels.groupby("request_id", sort=False)
    }
    ...
    for qid, branches in first.items():
        request = req[qid]
        pool = set(branches["baseline"])
        first_rank = sorted(pool, key=lambda x: (-branches["baseline"][x], x))
        pr = (
            prp[(prp.request_id.astype(str) == request) & (prp.passage_id.astype(str).isin(pool))]
        ...
        rows.append(
            {
                "request_id": request,
                "year": year[request],
                "query_words": int(text_len[request]),
                "judged_rate": float(cg.judged.mean()),
                "relevant_grade2plus": int(
                    rel[request] and sum(v >= 2 for v in rel[request].values())
                ),
                "bm25_ndcg": ndcg(bm, rel[request]),
                "first_ndcg": ndcg(first_rank, rel[request]),
                "prp_ndcg": ndcg(pr, rel[request]),
                "first_minus_prp": ndcg(first_rank, rel[request]) - ndcg(pr, rel[request]),
            }
        )
    frame = pd.DataFrame(rows)
    report = {"queries": len(frame), "by_year": {}}
    for y, group in frame.groupby("year"):
        report["by_year"][str(y)] = {
            "queries": len(group),
            **{
    ...
    report["correlations_overall"] = {
        col: float(frame[[col, "first_minus_prp"]].corr().iloc[0, 1])
        for col in ["query_words", "judged_rate", "relevant_grade2plus", "bm25_ndcg"]
    }

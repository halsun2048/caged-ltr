...
R3.1d 在不覆盖 R3.1c 原始缓存的前提下，将 184 个
`input_truncated=true` 的有序 prompt 写入独立审计清单，并固定
`max_input_tokens=1024` 重新计算。实际输入长度为 513—599 token，184 条全部
解除截断；只有所有响应完成且确认零截断后才读取 qrels 并重聚合。

| 项目 | 结果 |
|---|---:|
| 定向重算 prompt | 184 / 184 |
| 仍被截断 / 非法输出 | 0 / 0 |
| 相对 512-token 选择变化 | 49（26.63%） |
| 全排序变化 Query | 8 / 97 |
| Top10 内部顺序变化 Query | 2 / 97 |
| Top10 成员变化 Query | 0 / 97 |
| 最大候选 rank 位移 | 4 |
| 整体 NDCG@10 变化 | 0.000000 |

审计后的 DL19/DL20 NDCG@10 仍为 `0.709431/0.681258`，所有报告指标与 R3.1c
完全一致。平均 swap agreement 从 `0.794987` 变为 `0.795047`，变化也极小。
这表明截断会影响部分局部 pair 判断，不能把截断简单视为“逐条完全无影响”；但
双向 Allpair 聚合对本次极少量扰动足够稳健，Top10 成员和正式指标均未改变。

因此 R3.1c 的零截断疑点已经通过隔离敏感性审计关闭，可以正式认定 Top100
论文深度数值复现成功。下一步进入真实 Sliding-10 质量—成本对照，并统一使用
1024-token 上限。机器可读结果见
`reports/experiments/prp_r3_1d_truncation_audit.json`。

## R3.2 真实 Sliding-10 质量—成本对照

R3.2 使用 R3.1c 的完整有序 pair 缓存和 R3.1d 的 184 条修正覆盖层，对真实
FLAN-T5-XL 教师执行精确 Sliding-10 回放。由于 Allpair 已包含任意两个候选的
双向响应，而教师使用确定性 likelihood 判定，因此回放不需要新增 GPU 调用，
与重新请求同一批 pair 的选择结果等价。BM25 正序、逆序和固定 seed 42 随机顺序
的排名全部确定后，程序才读取 qrels。

| 初始顺序 | NDCG@10 | 相对 BM25 增益 | Allpair 增益保留率 | 逻辑成本 |
|---|---:|---:|---:|---:|
| BM25 | 0.668315 | +0.177067 | 87.44% | 20% |
| seed 42 随机 | 0.610654 | +0.119405 | 58.97% | 20% |
| 逆 BM25 | 0.557531 | +0.066282 | 32.73% | 20% |
| Allpair | 0.693747 | +0.202498 | 100% | 100% |

BM25 初排的 Sliding-10 在 DL19/DL20 分别达到 `0.680727/0.658432`，两个年度
都明显优于 BM25，说明它是有效的低成本重排器。但整体只保留 Allpair
`87.44%` 的增益，低于预注册的 `90%` 门槛，因此正式验收不通过。其逻辑
prompt 为 `192,060`，恰为 Allpair `960,300` 的 20%；由于相邻 pair 会在多轮
中重复，实际涉及的唯一有序 pair 只有 `100,438`。

初始顺序敏感性很强：随机和逆序相对 BM25 顺序分别下降
`0.057661/0.110785` NDCG@10。逆序场景即使 swap agreement 更高，也无法在
10 轮局部传播中把远离顶部的高质量候选充分提升，说明一致率高不等于最终排序好。
这也是 Sliding-10 不能替代 Allpair 作为正式教师的核心原因。

R3.2 的结论是：BM25 初始化 Sliding-10 保留为质量—成本消融和可选低预算方案，
R4 Instruction Distillation 的正式教师继续使用 Allpair；Sliding-10 作为学生
训练的教师来源对照。机器可读结果见
`reports/experiments/prp_r3_2_sliding10_cached_replay.json`。

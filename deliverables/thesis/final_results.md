...
CPU 统计产物已经生成：`final_statistics.json`（三 seed 汇总与 seed-level bootstrap CI）和 `head_torso_tail_delta.png`。由于冻结 JSON 没有逐 query 向量，该 bootstrap 是 seed-level，不应称为 query-level paired bootstrap。
